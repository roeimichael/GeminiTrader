...
class GraphSetup:
    ...
    def setup_graph(
        ...
        # Connect ALL analysts directly from START (they run in parallel!)
        for analyst_type in selected_analysts:
            workflow.add_edge(START, f"{analyst_type.capitalize()} Analyst")

        # Connect each analyst's tool chain
        for analyst_type in selected_analysts:
            current_analyst = f"{analyst_type.capitalize()} Analyst"
            current_tools = f"tools_{analyst_type}"
            current_clear = f"Msg Clear {analyst_type.capitalize()}"

            # Add conditional edges for current analyst (tools or proceed)
            ...
            # All analysts connect to aggregator (waits for all to finish)
            workflow.add_edge(current_clear, "Analyst Aggregator")

        # After all analysts complete, proceed to debate phase
