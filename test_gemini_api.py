...
def test_gemini_api():
    ...
    print("Rate limiting: 12 second delay between API calls")

    engine = GeminiAnalysisEngine(api_key, Settings.GEMINI_MODEL, rate_limit_delay=12)
    print("Engine initialized\n")

    test_tickers = Settings.DEFAULT_TICKERS[:3]
    print(f"Testing first 3 tickers: {', '.join(test_tickers)}")
    print(f"Expected runtime: ~2 minutes\n")

    results = []

    for i, ticker in enumerate(test_tickers, 1):
        print("-" * 80)
        print(f"TEST {i}/3: {ticker}")
        print("-" * 80)

        try:
            stock_data = get_stock_data(ticker)
            print(f"Stock data retrieved for {ticker}")
            print("Making 3 API calls (12s delays)...")
            ...
            if all(score is not None for score in scores.values()):
                ...
                print(f"\nSUCCESS: {ticker}")
                print(f"  Fundamental: {scores['fundamental_score']}/100")
                print(f"  Technical: {scores['technical_score']}/100")
                print(f"  Sentiment: {scores['sentiment_score']}/100")
            else:
                print(f"\nERROR: Some scores missing for {ticker}")
    ...
    try:
        ...
        print("\nMaking API call: Macro Analysis...")
    ...
    if results:
        ...
        for result in results:
            print(f"  {result['ticker']:6} - F: {result['fundamental']:3}/100, "
                  f"T: {result['technical']:3}/100, S: {result['sentiment']:3}/100")

        print(f"\nTotal API calls: {len(results) * 3 + 1}")
        print(f"  Stock analysis: {len(results)} tickers × 3 calls = {len(results) * 3}")
